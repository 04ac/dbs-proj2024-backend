...
async def create_category(category: Category):
    ...
    try:
        with connection:
            with connection.cursor() as cursor:
                cursor.execute("INSERT INTO Categories (category) VALUES (%s) RETURNING category_id", (category.category,))
...
async def create_customer(customer: Customer):
    ...
    try:
        with connection:
            with connection.cursor() as cursor:
                cursor.execute("INSERT INTO Customers (name, last_visit) VALUES (%s, %s) RETURNING customer_id", (customer.name, customer.last_visit,))
                customer_id = cursor.fetchone()[0]
        return {"customer_id": customer_id, "name": customer.name, "last_visit": customer.last_visit}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def create_main(main: Main):
    ...
    try:
        with connection:
            with connection.cursor() as cursor:
                cursor.execute("INSERT INTO Main (category_id, publisher_id, edition_id, format_id) VALUES (%s, %s, %s, %s) RETURNING book_id", (main.category_id, main.publisher_id, main.edition_id, main.format_id,))
                book_id = cursor.fetchone()[0]
        return {"book_id": book_id, "category_id": main.category_id, "publisher_id": main.publisher_id, "edition_id": main.edition_id, "format_id": main.format_id}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def create_physical_attr(physical_attr: PhysicalAttr):
    ...
    try:
        with connection:
            with connection.cursor() as cursor:
                cursor.execute("INSERT INTO Physical_Attr (x, y, z, w) VALUES (%s, %s, %s, %s) RETURNING book_id", (physical_attr.x, physical_attr.y, physical_attr.z, physical_attr.w,))
                book_id = cursor.fetchone()[0]
        return {"book_id": book_id, "x": physical_attr.x, "y": physical_attr.y, "z": physical_attr.z, "w": physical_attr.w}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def create_isbn(isbn: ISBN):
    ...
    try:
        with connection:
            with connection.cursor() as cursor:
                cursor.execute("INSERT INTO ISBN (isbn10, isbn13) VALUES (%s, %s) RETURNING book_id", (isbn.isbn10, isbn.isbn13,))
                book_id = cursor.fetchone()[0]
...
async def create_rating(rating: Rating):
    ...
    try:
        with connection:
            with connection.cursor() as cursor:
                cursor.execute("INSERT INTO Ratings (rating_avg, rating_count, for_ages) VALUES (%s, %s, %s) RETURNING rating_id", (rating.rating_avg, rating.rating_count, rating.for_ages,))
                rating_id = cursor.fetchone()[0]
        return {"rating_id": rating_id, "rating_avg": rating.rating_avg, "rating_count": rating.rating_count, "for_ages": rating.for_ages}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
# Create BookIssued
@app.post("/book-issued/")
async def create_book_issued(book_issued: BookIssued):
    connection = get_db_connection()
    try:
        with connection:
            with connection.cursor() as cursor:
                cursor.execute("INSERT INTO Book_Issued (customer_id, book_id) VALUES (%s, %s)", (book_issued.customer_id, book_issued.book_id,))
...
@app.get("/book-issued/")
async def get_all_book_issued():
    connection = get_db_connection()
    try:
        with connection:
            with connection.cursor() as cursor:
                cursor.execute("SELECT * FROM Book_Issued")
                book_issued_records = cursor.fetchall()
        return {"book_issued_records": book_issued_records}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def create_recommendation(recommendation: Recommendation):
    ...
    try:
        with connection:
            with connection.cursor() as cursor:
                cursor.execute("INSERT INTO Recommendations (customer_id, book_id) VALUES (%s, %s)", (recommendation.customer_id, recommendation.book_id,))
...
async def create_auth_book(auth_book: AuthBook):
    ...
    try:
        with connection:
            with connection.cursor() as cursor:
                cursor.execute("INSERT INTO Auth_Book (book_id, auth_id) VALUES (%s, %s)", (auth_book.book_id, auth_book.auth_id,))
